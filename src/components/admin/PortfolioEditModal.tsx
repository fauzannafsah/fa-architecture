...
export default function PortfolioEditModal({
  ...
  const { updatePortfolioProject, addPortfolioProject, deletePortfolioProject, uploadImage } =
    useContent();
  ...
  const handleSave = async () => {
    ...
    if (isNew) {
      ...
      await updatePortfolioProject(project.id, form);
    }
    setSaving(false);
    onClose();
  };
      ...
      >
        <div className="mb-6 flex items-center justify-between">
          <h3 className="font-serif text-xl text-foreground">
            {isNew ? "Tambah Proyek" : "Edit Proyek"}
          </h3>
          <button
            onClick={onClose}
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setForm((prev) => ({
                    ...prev,
                    size: e.target.value as "small" | "large",
                  }))
                }
                className="w-full rounded-lg border border-light-border p-3 text-sm focus:border-terracotta focus:outline-none"
              >
